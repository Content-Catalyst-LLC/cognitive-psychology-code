...
var requiredColumns = []string{
	...
	"prior_knowledge", "attention_score", "encoding_quality", "working_memory_load",
	"schema_strength", "retrieval_practice", "feedback_quality", "cognitive_load",
	"comprehension_score", "accuracy", "transfer_score", "retention_score",
	"response_time_ms", "learning_gain", "adaptive_application",
}

func main() {
	if len(os.Args) < 2 {
		log.Fatal("Usage: go run go/validator.go data/cognitive_learning_trials.csv")
	}
	...
	for rowNum, row := range rows[1:] {
		...
		checkRange(row, index, "encoding_quality", 0, 10, line, &errors)
		checkRange(row, index, "working_memory_load", 0, 10, line, &errors)
		checkRange(row, index, "schema_strength", 0, 10, line, &errors)
		checkRange(row, index, "feedback_quality", 0, 10, line, &errors)
		checkRange(row, index, "cognitive_load", 0, 10, line, &errors)
		checkRange(row, index, "comprehension_score", 0, 100, line, &errors)
		checkRange(row, index, "accuracy", 0, 1, line, &errors)
		checkRange(row, index, "transfer_score", 0, 100, line, &errors)
		checkRange(row, index, "retention_score", 0, 100, line, &errors)
		checkRange(row, index, "response_time_ms", 150, 1000000, line, &errors)
		checkRange(row, index, "learning_gain", -100, 100, line, &errors)
		checkRange(row, index, "adaptive_application", 0, 10, line, &errors)

		checkBinary(row, index, "retrieval_practice", line, &errors)
	}
