...
func main() {
	if len(os.Args) < 2 {
		log.Fatal("Usage: go run go/validator.go data/analogical_reasoning_trials.csv")
	}
	...
	for rowNum, row := range rows[1:] {
		...
		checkRange(row, index, "source_familiarity", 0, 10, line, &errors)
		checkRange(row, index, "target_novelty", 0, 10, line, &errors)
		checkRange(row, index, "surface_similarity", 0, 10, line, &errors)
		checkRange(row, index, "structural_similarity", 0, 10, line, &errors)
		checkRange(row, index, "relational_complexity", 0, 10, line, &errors)
		checkRange(row, index, "working_memory_load", 0, 10, line, &errors)
		checkRange(row, index, "inference_quality", 0, 100, line, &errors)
		checkRange(row, index, "schema_abstraction", 0, 10, line, &errors)
		checkRange(row, index, "confidence", 0, 10, line, &errors)
		checkRange(row, index, "response_time_ms", 150, 1000000, line, &errors)

		checkBinary(row, index, "analogical_cue", line, &errors)
		checkBinary(row, index, "mapping_accuracy", line, &errors)
		checkBinary(row, index, "transfer_success", line, &errors)
	}
